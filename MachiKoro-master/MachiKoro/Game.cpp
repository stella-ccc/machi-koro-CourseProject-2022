...
void Game::num_ai_player()//创造人物，并初始化发送卡片（一个WheatField，一个Bakery）
{
	...
	int num = 0;
	int ai_num = 0;
	cout << "Le nombre de joueurs:" << endl;
	cin >> num;
	if (num == 0)
	{
		cout << "numéro invalide, Veuillez réécrire" << endl;
		cout << "Le nombre de joueurs:" << endl;
		cin >> num;
	}
	cout << "Le nombre de joueurs ai:" << endl;
	cin >> ai_num;
...
void print_card(Card* c)//打印卡片，setw限定格式
{
	...
	if (c->get_string_color() == "jaune")//单独设置黄卡的打印
	{
...
int Game::player_input(string message)//玩家输入想要的操作
{
	...
	while (true)
	{
		...
		else if (input[0] == "acheter")
		{
			...
			{
				cout << "Pas de carte au numéro : " << stoi(input[1]) << endl;
				continue;
			}
...
void Game::purple_card_check()
{
	for (int i = 0; i < this->players[this->turn]->purple_cards.size(); i++)
	{
		...
		{
			cout << "Prenez jusqu'à 5 pièces de : ";
			for (int j = 0; j < this->players.size(); j++)
			{
		...
		{
			cout << "Échangez des cartes avec un joueur : " << endl;
			for (int j = 0; j < this->players.size(); j++)
			{
				cout << "Joueur  : " << j << endl;
				cout << "blue(1) : ";
			...
			int color_num;
			int card_num;
			cout << "Sélectionnez le joueur à prendre : ";
			cin >> player_num;
			cout << endl;

			cout << "Sélectionnez la couleur à prendre : ";
			cin >> color_num;
			cout << endl;

			cout << "Sélectionnez la carte à prendre : ";
			cin >> card_num;
			cout << endl;
			...
			cout << "Sélectionnez la couleur à donner : ";
			cin >> color_num;
			cout << endl;

			cout << "Sélectionnez la carte à donner : ";
			cin >> card_num;
			cout << endl;
...
void Game::buy_propery()
{
	...
	cout << "\n\n";
	if(version_old)
		cout << "Lequel voulez-vous acheter (no or 0-14 or TrainStation/ShoppingMall/AmusementPark/RadioTower)?" << endl;
	else
		...
		if (selection > 0)
		{
			...
			else if (Color::purple == this->slot[select][0]->get_color())
			{
				...
				for (int i = 0; i < this->players[this->turn]->purple_cards.size(); i++)
				{
					...
					{
						cout << "Vous avez déjà cet établissement" << endl;
						purp = false;
					}
				}
				if (purp)
